<script lang="ts">
	...
	onMount(() => {
		if (browser) {
			if (!localStorage.getItem('login')) goto('/login');
		}

		client = getNostrClient();

		// タイムライン購読
		timelineSub = client.rx_nostr
			.use(rxReqTimeline)
			...
			.subscribe({
				next: ({ event }) => {
					...
					try {
						...
						const pubkeys = npubResult
							?.map((npub) => decodeNostrURI(npub))
...
{#if kind === 'nevent' && nostrEvent}
	<Post event={nostrEvent!!} {profiles} nostr_ref={nostrRef} />
{:else if kind === 'note' && nostrEvent}
	<Post event={nostrEvent!!} {profiles} nostr_ref={nostrRef} />
{:else if kind === 'npub' && pubkey}
	<Profile {pubkey} {profiles} />
{/if}
